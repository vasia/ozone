...
public class DependencyConnectedComponents implements PlanAssembler, PlanAssemblerDescription {
	...
	public static final class FindCandidatesJoin extends MatchStub {
		...
		public void match(PactRecord vertexWithComponent, PactRecord edge, Collector<PactRecord> out) {
			this.result.setField(0, edge.getField(1, PactLong.class));	// output the target vertex
	...
	public static final class GroupCandidatesReduce extends ReduceStub{
		...
		public void reduce(Iterator<PactRecord> records, Collector<PactRecord> out) throws Exception {
			this.result.setField(0, records.next().getField(0, PactLong.class));
			out.collect(this.result);
		}
	}
	
	
	public static final class FindCandidatesDependencies extends MatchStub {
	...
	public Plan getPlan(String... args) {
		...
		WorksetIteration iteration = new WorksetIteration(0, "Dependency Connected Components Iteration");
		...
		//match the Workset with the dependency set to find candidates for recomputation
		MatchContract findCandidates = MatchContract.builder(FindCandidatesJoin.class, PactLong.class, 0, 0)
				.input1(iteration.getWorkset())
				.input2(edges)
				.name("Find Candidates for Recomputation")
				.build();
		
		ReduceContract groupCandidates = ReduceContract.builder(GroupCandidatesReduce.class, PactLong.class, 0)
				.input(findCandidates)
				.name("Group Candidates")
				.build();
		
		//find the dependencies of the candidates for recomputation
		MatchContract findCandidatesDependencies = MatchContract.builder(FindCandidatesDependencies.class, PactLong.class, 0, 1)
				.input1(groupCandidates)
				.input2(edges)
				.name("Find Dependencies of the Candidates for Recomputation")
				.build();
		...
		Plan plan = new Plan(result, "Dependency Connected Components");
