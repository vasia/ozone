...
public class CostModelConnectedComponentsTwoEnv implements ProgramDescription {
	...
	public static void main(String... args) throws Exception {
		...
		doBulkCostConnectedComponents(verticesWithInitialId, edges, maxIterations,
				numVertices, avgDegree, args[2]);		
		
		env.execute("Cost CC (bulk)");
		
		ExecutionEnvironment env2 = ExecutionEnvironment.getExecutionEnvironment();
		
		env2.setDegreeOfParallelism(1);

		doDepCostConnectedComponents(env.readCsvFile(args[2] + "_bulk").fieldDelimiter('\t').types(Long.class, Long.class), edges, maxIterations, args[2]);
		
		env2.execute("Cost CC (dep)");
		
	}
	
	public static DataSink<Tuple2<Long, Long>> doBulkCostConnectedComponents(DataSet<Tuple2<Long, Long>> verticesWithInitialId, 
			DataSet<Tuple2<Long, Long>> edges, int maxIterations, int numVertices, double avgDegree, String outPath) {
		...
		return bulkResult.writeAsCsv(outPath + "_bulk", "\n", "\t");
		
	}
				
		
	public static void doDepCostConnectedComponents(DataSet<Tuple2<Long, Long>> vertices, 
			DataSet<Tuple2<Long, Long>> edges, int maxIterations, String outPath) {
		...
		depResult.writeAsCsv(outPath + "_dep", "\n", " ");
	}
